...
final class BulkWriteHelper {
    ...
    static BulkWriteResult translateBulkWriteResult(final sources.mongo.com.mongodb.bulk.BulkWriteResult bulkWriteResult,
    ...
    static List<BulkWriteUpsert> translateBulkWriteUpserts(final List<sources.mongo.com.mongodb.bulk.BulkWriteUpsert> upserts,
                                                           final Decoder<DBObject> decoder) {
        List<BulkWriteUpsert> retVal = new ArrayList<BulkWriteUpsert>(upserts.size());
        for (sources.mongo.com.mongodb.bulk.BulkWriteUpsert cur : upserts) {
            retVal.add(new sources.mongo.com.mongodb.BulkWriteUpsert(cur.getIndex(), getUpsertedId(cur, decoder)));
        }
        return retVal;
    }

    private static Object getUpsertedId(final sources.mongo.com.mongodb.bulk.BulkWriteUpsert cur, final Decoder<DBObject> decoder) {
    ...
    static WriteConcernError translateWriteConcernError(final sources.mongo.com.mongodb.bulk.WriteConcernError writeConcernError) {
    ...
    static List<BulkWriteError> translateWriteErrors(final List<sources.mongo.com.mongodb.bulk.BulkWriteError> errors) {
        List<BulkWriteError> retVal = new ArrayList<BulkWriteError>(errors.size());
        for (sources.mongo.com.mongodb.bulk.BulkWriteError cur : errors) {
            retVal.add(new BulkWriteError(cur.getCode(), cur.getMessage(), DBObjects.toDBObject(cur.getDetails()), cur.getIndex()));
        }
        return retVal;
    }

    static List<sources.mongo.com.mongodb.bulk.WriteRequest> translateWriteRequestsToNew(final List<WriteRequest> writeRequests,
                                                                                final Codec<DBObject> objectCodec) {
        List<sources.mongo.com.mongodb.bulk.WriteRequest> retVal = new ArrayList<sources.mongo.com.mongodb.bulk.WriteRequest>(writeRequests.size());
